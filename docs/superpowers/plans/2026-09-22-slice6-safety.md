...
-- 카드 배치(match_candidates)가 매번 도는 조건이다. **선두 컬럼은 실제로 값을 좁히는 것**으로 둔다 —
-- `(status)` 단일 인덱스는 거의 모든 행이 'active' 라 스캔 폭이 안 줄어 플래너가 쓰지도 않는다.
-- match_candidates 의 where 절 순서와 맞춘다: 성별 <> 내 성별, 학교(→지역그룹), 15일 이내 접속.
create index profiles_active_pool_index
  on public.profiles (gender, university_id, last_active_at desc)
  where status = 'active'
    and not matching_paused
    and auto_hidden_at is null
    and withdrawn_at is null;
```

> 인덱스는 **붙인 뒤 `explain analyze` 로 실제로 쓰는지 확인**하고 안 쓰면 지운다. 지역그룹은
> `universities` 가 들고 있어 조인으로 걸리므로 프로필 쪽 선두는 `university_id` 다.
...
- [ ] **④ 와 ⑤ 의 순서가 중요하다.** 둘은 PostgREST 호출 두 번이라 한 트랜잭션이 아니다. 신고를 먼저
      ...
      사용자는 그냥 다시 신고하면 된다
- [ ] **매칭이 없거나 이미 닫힌 방을 신고 · 차단할 때**: 방 상태를 보지 않고 `blocks` 행만 세우고
      **200**. `match_participants` 가 없거나 이미 `left_at` 이면 그 단계를 건너뛴다 — 차단은 방이 아니라
      사람에 거는 것이라 방이 없다고 실패하면 14c(프로필)에서 온 차단이 막힌다
...
- [ ] **매칭이 없거나 이미 닫힌 방이어도 200.** `blocks` 행만 세우고 방 단계는 건너뛴다. B1 이 이 함수를
