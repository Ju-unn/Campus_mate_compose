...
> **[나는]** "나는 컴퓨터공학과, 공대 계열 학생이다. MBTI는 ENFP다. 얼굴은 강아지상이고 선한 인상이다.
...
예시의 김철수는 `impression_type = kind` 이고, 코드의 `IMPRESSION_PHRASES` 는 kind 를 "선한",
tofu(두부상) 를 "부드러운"으로 매핑한다 — 설계 §6.7 예시 문장의 "부드러운 인상"은 라벨과 어긋나서
위 예시를 코드 기준으로 고쳐 적는다(2026-09-20 리뷰 제안 5). 화면 라벨("선한상")과 별개로
문장용 표현을 두는 이유도 이것이다.
