...
| 27 | **학생증 인증 스키마 확정(2026-09-19~20)** — ① `profiles.department` 컬럼은 만들지 않고 기존 `major` 를 재사용, FastAPI 응답에서만 별칭. ② `profiles.student_number` 컬럼 신설(자기 입력, 학생증 대조 없음). ③ `student_verification` 이 `pending` → `verified`/`rejected` 로 확정되는 순간 트리거 `student_verification_finalized` 가 `student-id-temp` 최근 제출 파일을 즉시 삭제, 보관 기간 없음. ④ `verified` 확정 후에는 재제출을 FastAPI 가 409 로 막는다(`rejected` 는 계속 재제출 가능) | `profiles.student_number` 컬럼, `student_verification_finalized` 트리거(마이그레이션 `20260919181319`·`20260919181357`, 클라우드 미적용), §12-4 후속 | 설계 §7.3 학과·학번·재제출 서술, ERD.md §3·§9 |
