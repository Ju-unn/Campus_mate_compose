...
| 27 | **학생증 인증 스키마 확정(2026-09-19~20, ③ 2026-09-20 PR #43 으로 정정)** — ① `profiles.department` 컬럼은 만들지 않고 기존 `major` 를 재사용, FastAPI 응답에서만 별칭. ② `profiles.student_number` 컬럼 신설(자기 입력, 학생증 대조 없음). ③ `student_verification` 을 `verified`/`rejected` 로 확정한 직후 **FastAPI 가 Storage API로** `student-id-temp` 최근 제출 파일을 즉시 삭제한다(보관 기간 없음) — **왜 바뀌었나**: 애초 안이던 Postgres 트리거(`student_verification_finalized`, `delete from storage.objects`)는 `storage.objects` 메타 행만 지우고 실제 파일 객체는 고아로 남긴다(Supabase Storage 문서 — 파일 삭제는 Storage API를 거쳐야 함). 클라우드 적용 전에 발견돼 트리거 마이그레이션은 폐기·삭제했다. 삭제 실패는 인증 결과(성공)에 영향을 주지 않고 로그·디스코드 알림으로만 남으며, 고아 파일은 대시보드에서 손으로 지운다(SUPABASE.md §7). ④ `verified` 확정 후에는 재제출을 FastAPI 가 409 로 막는다(`rejected` 는 계속 재제출 가능) | `profiles.student_number` 컬럼(마이그레이션 `20260919181319`, 클라우드 적용됨), FastAPI Storage API 삭제(`student_verification_finalized` 트리거·마이그레이션 `20260919181357` 은 폐기·삭제), §12-4 후속 | 설계 §7.3·§7.4 학과·학번·삭제 방식 서술, ERD.md §3·§9, SUPABASE.md §1·§7 |
