...
-- 업로드는 앱이 FastAPI 로 보낸 사진을 FastAPI 가 service_role 키로 직접 올린다(서버측 업로드, 클라이언트는 이 버킷에 직접 접근하지 않는다).
-- 재검토 조회도 FastAPI 관리 기능으로만 한다.
-- 제한은 10MB · image/jpeg · image/png 다. 클라이언트가 업로드 전에 압축하고 JPEG 로 다시 인코딩한다(2026-09-14 사용자 결정).
-- 버킷 제한은 업로드 쪽이 신고한 content type 과 크기로만 막으므로(413 · 400), 파일 내용(매직 바이트)은 FastAPI 가 올리기 전에 검사하고 그 결과를 content type 으로 쓴다(조각 1 서버).
